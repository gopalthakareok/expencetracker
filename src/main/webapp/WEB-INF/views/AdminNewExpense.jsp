...
<body>
  	...
  	<main id="main" class="main">
    	...
    	<section class="section dashboard" style="min-height: 467px;">
            ...
            <div class="card-body">
              ...
              <!-- Vertical Form -->
              <form class="row g-3 mt-2" action="adminsaveexpense" method="post">
                 <div class="col-10">
                ...
                <div class="col-md-10">
                  ...
                  <select id="userId" class="form-select" name="userId" onchange="getUserRelatedData()" >
	...
	function getUserRelatedData(){
		...
		//url -> json REST 
		  ...
		  $.get( "getallaccountbyuserid/"+userId, function() {
			})
			  .done(function(data) {
			    console.log(data);
			    //fill the account 
			    $('#accountId').empty().append('<option selected="selected" value="-1">Select Account</option>')
			    
			    for (var i = 0; i < data.length; i++) {
    			  $('#accountId').append('<option value="' + data[i].accountId + '">' + data[i].title + '</option>');
 				 }
		  ...
		  $.get( "getallcategorybyuserid/"+userId, function() {
			  ...
			  $.get( "getallsubcategorybyuserid/"+userId, function() {
			})
			  .done(function(data) {
			    console.log(data);
			   ...
			   $.get( "getallvendorbyuserid/"+userId, function() {
			})
			  .done(function(data) {
			    console.log(data);
			    //fill the vendor 
			    $('#vendorId').empty().append('<option selected="selected" value="-1">Select Vendor</option>')
			    
			    for (var i = 0; i < data.length; i++) {
    			  $('#vendorId').append('<option value="' + data[i].vendorId + '">' + data[i].vendorName + '</option>');
 				 }
